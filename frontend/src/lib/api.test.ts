...
describe("api client auth handling", () => {
  ...
  it("prompts for a token on 401 and retries with the proxy-compatible app header", async () => {
    const fetchMock = vi
      .fn()
    ...
    expect(firstHeaders["X-RLE-Auth"]).toBeUndefined();
    expect(secondHeaders["X-RLE-Auth"]).toBe("secret-token");
  });

  it("does not send an app-token header when no token is set (public demo)", async () => {
    ...
    expect(headers["X-RLE-Auth"]).toBeUndefined();
  });
