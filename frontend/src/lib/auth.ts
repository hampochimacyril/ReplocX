/*
 * In-memory app-token auth for private ReplocX deployments.
 *
 * The public demo needs no token. A private instance sets RLE_PRIVATE_AUTH_TOKEN,
 * and every non-public /api route then returns 401 without a valid token. The
 * frontend sends it in X-RLE-Auth so a reverse proxy can use the standard
 * Authorization header for HTTP Basic authentication and translate the app
 * token to a Bearer header upstream. The token is requested on demand (the
 * first protected call) and held ONLY in memory for the session — it is never
 * written to localStorage, cookies, or the URL, so a reload re-prompts.
...
/** App-token header for the current token, or `{}` when none is set. */
export function authHeaders(): Record<string, string> {
  return authToken ? { "X-RLE-Auth": authToken } : {};
}
