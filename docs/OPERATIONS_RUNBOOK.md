...
| Data mode | `production` (`RLE_ANALYSIS_DATA_DIR=/analysis`, read-only mount) |
| Auth | Reverse-proxy Basic auth + app token (`RLE_REQUIRE_AUTH=1` + `RLE_PRIVATE_AUTH_TOKEN`) |
...
```bash
RLE_ANALYSIS_DATA_DIR=/analysis                  # read-only mount
RLE_ENABLE_ATLAS=1
RLE_ATLAS_DATA_DIR=/atlas                        # certified 4scen read-only mount
RLE_ATLAS_FIGURE_DIR=/atlas-figures              # certified f2v3_final read-only mount
RLE_REQUIRE_AUTH=1
...
The app exempts `/api/v1/health` and `/api/v1/openapi.json` from its token for
internal readiness, but the edge proxy protects every path with Basic auth.
The SPA sends its in-memory token as `X-RLE-Auth`; the proxy translates it to
`Authorization: Bearer <token>` upstream. The token is compared with
`hmac.compare_digest` (`backend/auth.py`).

## 2. Health monitoring

The internal readiness probe is `GET /api/v1/health` (no app token); an
edge-origin probe still supplies Basic auth. A healthy production instance
returns:
...
disk. National and Atlas datasets are immutable read-only mounts, reproducible
from their certified sources, so disaster recovery is scenarios-only.
...
Run from an allowlisted host (`$BASIC_USER` / `$BASIC_PASSWORD` are the proxy
credentials, `$T` is the app token, and `$U` is the private service URL):

```bash
curl -s -o /dev/null -w '%{http_code}\n' "$U/atlas"                                # 401 at proxy
curl -s -u "$BASIC_USER:$BASIC_PASSWORD" "$U/api/v1/health" | python3 -m json.tool
curl -s -u "$BASIC_USER:$BASIC_PASSWORD" -o /dev/null -w '%{http_code}\n' \
     "$U/api/v1/results/provenance"                                                  # 401 at app
curl -s -u "$BASIC_USER:$BASIC_PASSWORD" -H "X-RLE-Auth: $T" \
     "$U/api/v1/results/provenance" | python3 -m json.tool                           # 200; 720/R9/f2v3
curl -s -u "$BASIC_USER:$BASIC_PASSWORD" -H "X-RLE-Auth: $T" \
     "$U/api/v1/equity/profiles" | python3 -m json.tool                             # 200; READY/review explicit
```

Also load the UI over HTTPS, confirm Basic auth is demanded before the shell,
enter the independent app token when prompted, and verify the certified Atlas
provenance/equity state.
...
| Health `degraded` / API 503 | data dir not resolving | Check the read-only analysis/Atlas mounts and env paths; redeploy. |
